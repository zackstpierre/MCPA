...
grating_y = [1:30,60:90,120:150];
grating_x = [60:90,125:150];


epi{1} = ones(nx{1},ny{1})*c_eps_0;
%epi{1}(grating,55:95)= c_eps_0*11.3;  %"inclusion"

for i = 1:length(grating_y)
    for j = 1:length(grating_x)
        if (grating_x(j) >=60) && (grating_x(j) <= 90) && ~(grating_y(i) < 60) && ~(grating_y(i) > 90)
             epi{1}(grating_x(j),grating_y(i))= c_eps_0;
        
        else
            epi{1}(grating_x(j),grating_y(i))= c_eps_0*11.3;
        end
    end
end
